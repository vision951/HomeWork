# Банкровские транзакции

## Описание проекта
Проект предоставляет набор функций для обработки списка банковских транзакций:
- Фильтрация по статусу
- Сортировка по дате
- Маскировка номеров карт и счетов
- Форматирование дат
- фильтрация транзакций по валюте операций
- генерации описаний транзакций
- генерации номера банковский карт
- загружает данные о транзакциях
- конвертирует валюту в рубли


## Установка

1. Склонируйте репозиторий:
   ```
   git clone git@github.com:vision951/HomeWork.git
2. Установка зависимостей
    ```
   poetry install
3. Активация виртуального окружения:
    ```
   poetry shell
4. Установка линтеров flake8, mypy, black, isort
   ```
   poetry add flake8 --group lint
   poetry add mypy --group lint
   poetry add black --group lint
   poetry add isort --group lint
5. Установка необходимых инструментов для тестирования:
   ```
   poetry add --dev pytest-cov
   poetry add --dev coverage


##  Проверки оформления кода

Проект проходит следующие виды проверок:

### 1. Статическая проверка типов (mypy)
      ```
      poetry run mypy src/
### 2. Проверка стилея(flake8)
      ```
      poetry run flake8 src/

## Тестирование
Проект покрыт автоматическими тестами. Проверены следующие сценарии:

### masks.py:
1. Функция get_mask_card_number:
- Тестирование правильности маскирования номера карты.
- Проверка работы функции на различных входных форматах номеров карт, включая граничные случаи и нестандартные длины номеров.
- Проверка, что функция корректно обрабатывает входные строки, где отсутствует номер карты.
2. Функция get_mask_account:
- Тестирование правильности маскирования номера счета.
- Проверка работы функции с различными форматами и длинами номеров счетов.
- Проверка, что функция корректно обрабатывает входные данные, где номер счета меньше ожидаемой длины.

### widget.py
1. Функция mask_account_card:
- Тесты для проверки, что функция корректно распознает и применяет нужный тип маскировки в зависимости от типа входных данных (карта или счет).
- Параметризованные тесты с разными типами карт и счетов для проверки универсальности функции.
- Тестирование функции на обработку некорректных входных данных и проверка ее устойчивости к ошибкам.
2. Функция get_date:
- Тестирование правильности преобразования даты.
- Проверка работы функции на различных входных форматах даты, включая граничные случаи и нестандартные строки с датами.
- Проверка, что функция корректно обрабатывает входные строки, где отсутствует дата.
### processing.py
1. Функция filter_by_state:
- Тестирование фильтрации списка словарей по заданному статусу state.
- Проверка работы функции при отсутствии словарей с указанным статусом state в списке.
- Параметризация тестов для различных возможных значений статуса state. 
2. Функция sort_by_date:
- Тестирование сортировки списка словарей по датам в порядке убывания и возрастания.
- Проверка корректности сортировки при одинаковых датах.
- Тесты на работу функции с некорректными или нестандартными форматами дат.
### generators.py
1. Функция filter_by_currency:
- проверка работы функции при получение корректных данных;
- проверка обработки случаев отсутствия валюты, пустого списка транзакций или без валютных операций.
2. Функция transaction_descriptions:
- проверка работы функции при получение корректных данных;
- проверка с использованием различных количеств транзакций, пустого списка, некоректных данных.
3. Функция card_number_generator:
- проверка работы функции при получение корректных данных в заданном диапазоне;
- проверка корректности форматирования карт;
- проверка корректной обработки крайних значений и правильного завершения генерации
### decorators.py
- Проверка успешного выполнения функции с логированием в консоль и обработки ошибки;
- Проверка успешного выполнения с логированием в файл и обработки ошибки;
- Проверка работы декоратора без аргументов
### utils.py
- Проверка успешной загрузки транзакции из файла;
- Проверка обработки ошибок, когда файл отсутствует или получены некорректные данные.
### external_api.py
- Проверка конвертации транзакции в разной валюте("USD", "RUB");
- Проверка обработки ошибки API;
- Проверка обработки пустого списка, а так же неверного формата суммы транзакции.
### Запуск тестов с подсчетом coverage и генерацией HTML-отчета:
    ```
      poetry run pytest --cov=your_package --cov-report=html
      poetry run pytest --cov=src/   # для src/

### Результаты тестирования
      Name                Stmts   Miss  Cover
      ---------------------------------------
      src/__init__.py           0      0   100%
      src/decorators.py        23      0   100%
      src/external_api.py      33      1    97%
      src/generators.py        14      0   100%
      src/masks.py             14      0   100%
      src/processing.py        16      0   100%
      src/utils.py              8      0   100%
      src/widget.py            32      0   100%
      -----------------------------------------
      TOTAL                   140      1    99%

      ---------------------------------------
      TOTAL                 101      0   100%



## Примеры работы функций
### Маскировка карты → "Visa Platinum 1234 56** **** 3456"
      masked_card = mask_number("Visa Platinum 1234567890123456") 

### Маскировка счета → "Счет **3456"
      account_masked = mask_number("Счет 1234567890123456")

### Форматирование даты → 1.03.2024
      formatted_date = get_date("2024-03-11T02:26:18.671407")

### Фильтрация выполненных транзакций
      executed = filter_by_state(transactions, state="EXECUTED")

### Сортировка по дате (новые сначала)
      sorted_transactions = sort_by_date(transactions)

### Фильтрация транзакций по заданной валюте
      usd_transactions = filter_by_currency(transactions, "USD")

### Генератор описаний каждой операции
      descriptions = transaction_descriptions(transactions)
      for _ in range(5):
      print(next(descriptions))

### Генератор номеров банковских карт
      for card_number in card_number_generator(1, 5):
      print(card_number)

### Декорирование функции 
      @log(filename="mylog.txt")
      def my_function(x, y):
          return x + y

### Загрузка данных о транзакциях
      load_transactions("operations.json")

### Конвертация суммы транзакции
      convert_transaction_to_rub(transaction)